...
7. 좋아요
   ...
   ![image-20230216144003426](md-images/image-20230216144003426.png)

   8. 댓글

      ![image-20230216154259772](md-images/image-20230216154259772.png)

      8. 알람

         ![image-20230216161358057](md-images/image-20230216161358057.png)
...
1. build.gradle 에서 build하는 주석처리
...
1. Controller
   1. 성공 -> isOk()
   2. 로그인 안함 -> isUnauthorized()
   3. 포스트글이 없음 -> isNotFound()
...
1. Controller
   1. 댓글 작성
      1. 성공 -> isOk()
      2. 로그인 안함 -> isUnauthorized()
      3. 포스트 글이 없음 -> isNotFound()
   2. 댓글 리스트 목록
      1. 성공 -> isOk()
      2. 포스트 글이 없음 -> isNotFound()
...
1. AuthnticationConfig

   1. request filtering 추가
...
2. Service
   ...
   1. like()
      ...
      1. 포스트 글 존재여부 확인
         1. 있으면 post Entity 가져오기
         2. 없으면 POST_NOT_FOUND 에러처리
      2. 로그인 여부 확인
         1. 했으면 user Entity 가져오기
         2. 안했으면 USER_NOT_FOUND 에러처리
      3. 좋아요 한 사람당 1개 확인
         1. 이미 했으면 ALREADY_LIKED 에러처리
      4. save()
   ...
   2. likeCount
      ...
      2. countByPost()로 좋아요 갯수 반환

         1. Repository에서 구현

            @Query에서 count(*) 사용해서 구현

            findyAllByPost()로 하면 필요없는 정보까지 가져옴

3. enum ErroCode
   1. ALREADY_LIKED 추가
4. Entity
   1. LikeEntity
      1. id,user,post(many to one join)
      2. metadata(수정일, 삭제일, 작성일)
      3. 변환(postEntity + userEntity -> likeEntity)
...
1. Controller
   ...
   1. 댓글 작성
      1. postMapping
      2. postId, commentRequest, authentication
      3. getComments service
      4. Response 규격화
   2. 댓글 리스트 목록
      1. getMapping
      2. postId, pageable authentication 가져오기
      3. comment service 
      4. Response 규격화
...
3. dto
   ...
   3. commentResponse
      ...
      3. 변환(dto -> response dto)

         

## Index

index 안걸어주면 findAllByEntity시 느림
...
2. Service
   ...
   UserService에서 alarmList service 구현

   comment service 마지막에 alarm save

   like service 마지막에 alarm save
...
3. Entity
   ...
   3. 알람 정보
      ...
      2. AlarmArgs
         ...
         jsonb type으로 지정 -> 값의 타입이 변경 자주 됨 + idx

         1. fromUserId
         2. targetId

4. jsonb type dependecy 추가

   JPA에 jsonb 타입이 없어서 추가
